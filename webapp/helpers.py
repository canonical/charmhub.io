...
# Solutions helpers
SOLUTIONS_API_BASE = os.getenv("SOLUTIONS_API_BASE", "http://localhost:5000/api")
...
def get_publisher_solutions(username):
    ...
    try:
        ...
        resp = session.get(f"{SOLUTIONS_API_BASE}/publisher/solutions", headers=headers, timeout=5)
        if resp.status_code == 200:
            solutions = resp.json()
