...
from canonicalwebteam.flask_vite.extension import vite_import as flask_vite_import
...
def _rewrite_module_specifiers(
    ...
    def replace_specifier(match: re.Match) -> str:
        ...
        if resolved_specifier in known_module_urls:
            return (
                f"{match.group('prefix')}{resolved_specifier}"
...
def _inline_script_import(entrypoint: str) -> Markup:
    ...
    import_map_script = (
        ...
        f"{_escape_inline_script(json.dumps(import_map, separators=(',', ':')))}"
        "</script>"
    )
    entry_script = (
