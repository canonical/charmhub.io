import { initialiseContactForm, fetchForm, updateHash, setProductContext } from '../dynamic-contact-form';
...
describe('Dynamic contact form', () => {
  ...
  test('initialiseContactForm sets up event listeners and opens form on hash match', async () => {
    ...
    expect((global as any).ga).toHaveBeenCalledWith("send", "event", "interactive-forms", "open", window.location.pathname);
  ...
  test('fetchForm updates form container with fetched HTML', async () => {
    ...
    (global.fetch as jest.Mock).mockResolvedValue({
      text: () => Promise.resolve('<form>%% formid %% %% lpId %% %% returnURL %% %% lpurl %%</form>')
    });

    await fetchForm(mockData);

    const formContainer = screen.getByText('123 456 http://example.com http://example.com/lp');
  ...
  test('setProductContext sets the product context correctly', () => {
    ...
    const contactButton = document.querySelector('.js-invoke-modal') as HTMLAnchorElement;
