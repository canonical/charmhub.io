...
export async function initialiseContactForm() {
  ...
  const contactButtons = document.querySelectorAll<HTMLAnchorElement>(".js-invoke-modal");
...
export async function fetchForm(formData: DOMStringMap, contactButton?: HTMLAnchorElement | undefined) {
  try {
...
export function setProductContext(contactButton?: HTMLAnchorElement | undefined) {
  const product = contactButton ? new URL(contactButton.href).searchParams.get("product") || window.location.pathname.split("/").slice(1).join("-") || "global" : window.location.pathname.split("/").slice(1).join("-") || "global";
...
export function initialiseForm() {
  ...
  modalPaginationButtons?.forEach(button => {
    button.addEventListener("click", (e) => {
      e.preventDefault();
      contactIndex += button.classList.contains("pagination__link--previous") ? -1 : 1;
      setState(contactIndex);
      // @ts-ignore
      ga("send", "event", "interactive-forms", "goto:" + contactIndex, window.location.pathname);
    });
  });

  otherContainers.forEach(container => {
    const checkbox = container.querySelector<HTMLInputElement>(".js-other-container__checkbox")!;
    const input = container.querySelector<HTMLElement>(".js-other-container__input")!;
  ...
  function render() {
    ...
    const currentContent = contactModal?.querySelector(".js-pagination--" + contactIndex) as HTMLElement;
  ...
  function createMessage() {
    ...
    formFields.forEach(field => {
      ...
      const inputs = field.querySelectorAll<HTMLInputElement>("input, textarea");
      ...
      inputs.forEach(input => {
        switch (input.type) {
          ...
          case "checkbox":
            if (input.checked) {
              const subSection = input.closest('[class*="col-"]')?.querySelector(".js-sub-section");
              const label = field.querySelector<HTMLLabelElement>(`label[for=${input.id}]`);
              if (label) message += ", " + (subSection?.textContent + ": " || "") + label.textContent;
            }
            break;
          case "text":
          case "textarea":
            if (!input.classList.contains("mktoField") && input.value) message += ", " + input.value;
            break;
        }
      });
      message += "\r\n\r\n";
