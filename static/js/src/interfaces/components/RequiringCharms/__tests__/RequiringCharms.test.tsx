...
describe('RequiringCharms', () => {
  ...
  test('renders RequiringCharms with requirers and other charms correctly', () => {
    const { container } = render(<RequiringCharms interfaceData={mockRequiringCharmsData} isCommunity={false} />);
    ...
    expect(iframes[0].getAttribute('src')).toBe('/Requirer1/embedded/interface');
    expect(iframes[1].getAttribute('src')).toBe('/Requirer2/embedded/interface');
  ...
  test('renders RequiringCharms with no requirers correctly', () => {
    render(<RequiringCharms interfaceData={noCharmsData} isCommunity={false} />);
  ...
  test('does not render "How to test a charm" link when isCommunity is true', () => {
    const { container } = render(<RequiringCharms interfaceData={mockRequiringCharmsData} isCommunity={true} />);
